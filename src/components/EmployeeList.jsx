...
const EmployeeList = ({ employees, onAddEmployeeClick, onEditEmployee, onDeleteEmployee, isStepCompleted, onToggleStepCompleted }) => {
  ...
  const activeCount = validEmployees.filter(emp => emp && emp.status === 'ativo').length;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (activeOptionsId !== null && listContainerRef.current && !listContainerRef.current.contains(event.target)) {
    ...
    <div className="w-full lg:flex-grow flex flex-col h-full rounded-2xl">
      <section className="bg-white rounded-2xl flex flex-col flex-grow card-shadow">
        ...
        <div className="flex-grow p-6">
          <button onClick={onAddEmployeeClick} className="w-full h-[56px] bg-white border border-[#1976D2] text-[#1976D2] text-base font-medium rounded-xl hover:bg-[#1976D2]/10 transition-colors flex items-center justify-center gap-2 mb-8">
        ...
        <div className="p-6 bg-white border-t border-[#E0E0E0] flex items-center justify-end rounded-b-2xl">
          <div className="flex items-center gap-4">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={isStepCompleted} onChange={(e) => onToggleStepCompleted(e.target.checked)} className="sr-only peer"/>
              <div className="w-20 h-8 bg-[#E0E0E0] rounded-full flex items-center justify-between px-2 text-xs font-bold peer-checked:bg-[#E0E0E0] transition-colors">
                <span className={isStepCompleted ? 'text-[#212121]' : 'text-transparent'}>Sim</span>
                <span className={!isStepCompleted ? 'text-[#212121]' : 'text-transparent'}>Não</span>
              </div>
