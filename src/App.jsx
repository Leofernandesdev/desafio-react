...
function App() {
  ...
  const handleDeleteEmployee = (employeeIdToDelete) => {
    setEmployees((prevEmployees) =>
      prevEmployees.filter((emp) => emp.id !== employeeIdToDelete)
    );
          ...
          <div className="w-full flex justify-between mt-auto pt-4">
            {currentStep > 1 ? (
              <button
                ...
                className="bg-[#1976D2] text-white font-roboto font-bold text-sm w-[194px] h-[32px] flex items-center justify-center rounded-xl hover:bg-blue-800 transition-colors"
              >
                Passo anterior
              </button>
            ) : (
            ...
            {currentStep < stepperSteps.length && (
              <button
                ...
                className={`w-[194px] h-[32px] flex items-center justify-center font-roboto font-bold text-sm rounded-xl transition-colors ${
                  isNextButtonDisabled
                    ? "bg-[#E0E0E0] text-[#757575] cursor-not-allowed"
                    : "bg-[#1976D2] hover:bg-blue-800 text-white"
                }`}
              >
                Próximo passo
              </button>
